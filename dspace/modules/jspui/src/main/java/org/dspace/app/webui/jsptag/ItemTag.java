...
public class ItemTag extends TagSupport
{
    ...
    private void render() throws IOException, SQLException, DCInputsReaderException
    {
        ...
        for (String field : metadataFields)
        {
            ...
            if (style != null)
            {
                ...
                //modified by dspanos
                isDuration = style.contains("duration");
                //end dspanos
                ...
                for (MetadataValue val : values)
                {
                    if (val != null && val.getValue() != null)
                    {
                        ...
                        //modified by dspanos
                        	...
                        	out.print(displayDuration(val.getValue(), sessionLocale.toString()));
                        }
                        //end dspanos
                        else if (isResolver)
                        {
    ...
    /**
     * Formats a duration string of the form PThHmMsS in human-readable format
     * 
     * @param duration
     * @param locale
     * @return
     */
    private String displayDuration(String duration, String locale){
    	Matcher matchDuration = Pattern.compile("^PT(\\d*)H(\\d*)M(\\d*)S").matcher(duration);
    	if (matchDuration.matches()){
    		int hours = Integer.parseInt(matchDuration.group(1));
    		int minutes = Integer.parseInt(matchDuration.group(2));
    		int seconds = Integer.parseInt(matchDuration.group(3));
    		if ("el".equals(locale))
    			return (hours==0 ? "" : hours+"ω") + (minutes==0 ? "" : minutes+"λ") + (seconds==0 ? "" : seconds+"δ");
    		else return (hours==0 ? "" : hours+"h") + (minutes==0 ? "" : minutes+"m") + (seconds==0 ? "" : seconds+"s");
    	}
    	return duration;
    }
}
