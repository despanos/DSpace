...
public class BitstreamServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        if (dso != null && dso.getType() == Constants.ITEM && sequenceID >= 0)
        {
            ...
            //modified by dspanos
            List<MetadataValue> accessValues = itemService.getMetadata(item, "heal", "access", null, Item.ANY);
            if (accessValues.size() > 0 && "campus".equalsIgnoreCase(accessValues.get(0).getValue())){
            	String campusIP = ConfigurationManager.getProperty("embargo.ip");
            	if (campusIP!=null && !authorizeService.authorizeActionBoolean(context, item, Constants.ADMIN) && !request.getRemoteAddr().startsWith(campusIP))
            		throw new AuthorizeException("The bitstream you requested cannot be viewed from your current location");
            }
            //end dspanos
