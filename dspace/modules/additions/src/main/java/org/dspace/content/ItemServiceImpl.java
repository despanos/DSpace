...
//                    modified = true;
    ...
    public void adjustBundleBitstreamPolicies(Context context, Item item, Collection collection) throws SQLException, AuthorizeException {
        ...
        //modified by dspanos
        List<ResourcePolicy> defaultItemPolicies = authorizeService.getPoliciesActionFilter(context, item, Constants.DEFAULT_BITSTREAM_READ);
        //end dspanos
        ...
        for (Bundle mybundle : bunds) {
            ...
            //modified by dspanos
            if (!isGroupAccessSet(item))
            	addDefaultPoliciesNotInPlace(context, mybundle, defaultCollectionPolicies);
            else addDefaultPoliciesNotInPlace(context, mybundle, defaultItemPolicies);
            //end dspanos
            ...
            for(Bitstream bitstream : mybundle.getBitstreams())
            {
                ...
                else addDefaultPoliciesNotInPlace(context, bitstream, defaultItemPolicies);
                //end dspanos
            }
        }
    }
    ...
    public void adjustItemPolicies(Context context, Item item, Collection collection) throws SQLException, AuthorizeException {
        ...
        try {
            ...
            //modified by dspanos
            // add default policies only if not already in place, unless a group policy has already been specified in the access step
            if (!isGroupAccessSet(item))
    ...
    //added by dspanos
    private boolean isGroupAccessSet(Item item){
    	List<MetadataValue> accessValues = getMetadata(item, "heal", "access", null, Item.ANY);
        if (accessValues.size() > 0 && "account".equalsIgnoreCase(accessValues.get(0).getValue()))
        	return true;
        return false;
    }
    //end dspanos
