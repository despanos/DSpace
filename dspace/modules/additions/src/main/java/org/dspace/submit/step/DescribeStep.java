...
public class DescribeStep extends AbstractProcessingStep
{
    ...
    protected void readNames(Context context, HttpServletRequest request, Item item,
            String schema, String element, String qualifier, boolean repeated, String lang, boolean hasLanguageTag) throws SQLException {
        ...
        for (int i = 0; i < lasts.size(); i++)
        {
            ...
            if ((l != null) && !((l.trim()).equals("")))
            {
                ...
                //modified by dspanos
                if (hasLanguageTag && !repeated && i == 0) 
                {
                ...
                {
                    lang = request.getParameter(metadataField + "_" + (i+1) + "[lang]");
                }
                //end dspanos
                ...
                if (isAuthorityControlled)
                {
                    ...
                    {
                    	//modified by dspanos
                        itemService.addMetadata(context, item, schema, element, qualifier, lang,
