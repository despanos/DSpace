...
public class AccessStep extends AbstractProcessingStep
{
    ...
    public int doProcessing(Context context, HttpServletRequest request,
        ...
        //modified by dspanos
        //String access = request.getParameter("heal_access");
        //store embargo date in heal.access so that EmbargoServiceImpl parses it
        String embargoDate = request.getParameter("embargo_until_date");
        if (embargoDate!=null){
        	 itemService.clearMetadata(context, item, "heal", "access", null, Item.ANY);
             itemService.addMetadata(context, item, "heal", "access", null, null, embargoDate);
        }
//        String accessText = request.getParameter("heal_accessText");
//        if (accessText!=null){
//        	 itemService.clearMetadata(context, item, "heal", "accessText", null, Item.ANY);
//             itemService.addMetadata(context, item, "heal", "accessText", null, null, accessText);
//        }
        String groupID = request.getParameter("heal_access_groups");
        Group accessGroup = null;
        if (groupID!=null){
        	try{
        		accessGroup=groupService.find(context, UUID.fromString(groupID));
        		//remove all policies due to back and front in the submission process
        		authorizeService.removePoliciesActionFilter(context, item, org.dspace.core.Constants.READ);
        		authorizeService.addPolicy(context, item, org.dspace.core.Constants.READ, accessGroup);
            }catch (NumberFormatException nfe){
                return STATUS_ERROR_SELECT_GROUP;
            }
        }
        ...
        //modified by dspanos
        if(!isAdvancedFormEnabled && accessGroup==null){
        //end dspanos
